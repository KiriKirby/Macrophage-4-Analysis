macro "巨噬細胞画像 四要素解析 / Macrophage Four-Factor Analysis / マクロファージ四要素解析" {
    ...
    // -----------------------------------------------------------------------------
    // 関数: stirRandomByTime
    // 概要: 時刻由来の値で乱数系列を軽く撹拌する（デバッグ用）。
    // 引数: tag (number)
    // 戻り値: なし
    // -----------------------------------------------------------------------------
    function stirRandomByTime(tag) {
        seed = getTime();
        seed = seed + tag * 131;
        steps = seed - floor(seed / 97) * 97;
        steps = floor(steps);
        if (steps < 1) steps = 1;
        i = 0;
        while (i < steps) {
    ...
    function isParamSpecKey(key) {
        if (key == "mina") return 1;
        if (key == "maxa") return 1;
        if (key == "circ") return 1;
        if (key == "allowclumps") return 1;
        if (key == "centerdiff") return 1;
        if (key == "bgdiff") return 1;
        if (key == "smallratio") return 1;
        if (key == "clumpratio") return 1;
        if (key == "exclenable") return 1;
        if (key == "exclmode") return 1;
        if (key == "exclthr") return 1;
        if (key == "exclstrict") return 1;
        if (key == "exclsizegate") return 1;
        if (key == "exclmina") return 1;
        if (key == "exclmaxa") return 1;
        if (key == "minphago") return 1;
        if (key == "pixelcount") return 1;
        if (key == "strict") return 1;
        if (key == "roll") return 1;
        if (key == "roisuffix") return 1;
        if (key == "fluotarget") return 1;
        if (key == "fluonear") return 1;
        if (key == "fluotol") return 1;
        if (key == "fluoexclenable") return 1;
        if (key == "fluoexcl") return 1;
        if (key == "fluoexcltol") return 1;
        return 0;
    }
    ...
    function buildParamSpecString() {
        spec = "";
        spec = appendParamSpec(spec, "minA", "" + beadMinArea);
        spec = appendParamSpec(spec, "maxA", "" + beadMaxArea);
        spec = appendParamSpec(spec, "circ", "" + beadMinCirc);
        spec = appendParamSpec(spec, "allowClumps", "" + allowClumpsTarget);
        spec = appendParamSpec(spec, "centerDiff", "" + centerDiffThrUI);
        spec = appendParamSpec(spec, "bgDiff", "" + bgDiffThrUI);
        spec = appendParamSpec(spec, "smallRatio", "" + smallAreaRatioUI);
        spec = appendParamSpec(spec, "clumpRatio", "" + clumpMinRatioUI);
        spec = appendParamSpec(spec, "exclEnable", "" + useExclUI);
        spec = appendParamSpec(spec, "exclMode", "" + exclMode);
        spec = appendParamSpec(spec, "exclThr", "" + exThrUI);
        spec = appendParamSpec(spec, "exclStrict", "" + useExclStrictUI);
        spec = appendParamSpec(spec, "exclSizeGate", "" + useExclSizeGateUI);
        spec = appendParamSpec(spec, "exclMinA", "" + exclMinA_UI);
        spec = appendParamSpec(spec, "exclMaxA", "" + exclMaxA_UI);
        spec = appendParamSpec(spec, "minPhago", "" + useMinPhago);
        spec = appendParamSpec(spec, "pixelCount", "" + usePixelCount);
        spec = appendParamSpec(spec, "strict", strictKeyFromChoice(strictChoice));
        spec = appendParamSpec(spec, "roll", "" + rollingRadius);
        spec = appendParamSpec(spec, "roiSuffix", "" + roiSuffix);
        if (HAS_FLUO == 1) {
            spec = appendParamSpec(spec, "fluoTarget", "" + fluoTargetRgbStrUI);
            spec = appendParamSpec(spec, "fluoNear", "" + fluoNearRgbStrUI);
            spec = appendParamSpec(spec, "fluoTol", "" + fluoTolUI);
            spec = appendParamSpec(spec, "fluoExclEnable", "" + fluoExclEnableUI);
            spec = appendParamSpec(spec, "fluoExcl", "" + fluoExclRgbStrUI);
            spec = appendParamSpec(spec, "fluoExclTol", "" + fluoExclTolUI);
        }
        return spec;
    }
    ...
    function applyParamSpec(spec, stage) {
        ...
        req = newArray(
            ...
            "strict", "roll", "roisuffix"
        );
        if (HAS_FLUO == 1) {
            req2 = newArray(req.length + 6);
            j = 0;
            while (j < req.length) {
                req2[j] = req[j];
                j = j + 1;
            }
            req2[j] = "fluotarget"; j = j + 1;
            req2[j] = "fluonear"; j = j + 1;
            req2[j] = "fluotol"; j = j + 1;
            req2[j] = "fluoexclenable"; j = j + 1;
            req2[j] = "fluoexcl"; j = j + 1;
            req2[j] = "fluoexcltol";
            req = req2;
        }
        i = 0;
        while (i < req.length) {
            if (hasParamSpecKey(keys, req[i]) == 0) {
                msg = replaceSafe(T_err_param_spec_missing, "%s", req[i]);
                showParamSpecError(msg);
                return 0;
            }
            i = i + 1;
        ...
        roiSuffix = findParamSpecValue(keys, vals, "roisuffix");

        if (HAS_FLUO == 1) {
            fluoTargetRgbStrUI = findParamSpecValue(keys, vals, "fluotarget");
            fluoNearRgbStrUI = findParamSpecValue(keys, vals, "fluonear");
            ...
            fluoExclRgbStrUI = findParamSpecValue(keys, vals, "fluoexcl");
    ...
    function calcTuningScore(idxList, pnIndexA, timeIndexA, nPn, nT) {
        ...
        while (p < nPn) {
            ...
            if (ratios.length >= 2) {
                ...
                variance = (sum2 / ratios.length) - mean * mean;
    ...
    function adjustParamsForTuning(ratioMean, ratioCv, iter) {
        ...
        line = T_log_tune_adjust;
        line = replaceSafe(line, "%min", "" + beadMinArea);
        line = replaceSafe(line, "%max", "" + beadMaxArea);
        line = replaceSafe(line, "%circ", "" + beadMinCirc);
        line = replaceSafe(line, "%diff", "" + centerDiffThrUI);
        line = replaceSafe(line, "%bg", "" + bgDiffThrUI);
        line = replaceSafe(line, "%small", "" + smallAreaRatioUI);
        line = replaceSafe(line, "%clump", "" + clumpMinRatioUI);
        line = replaceSafe(line, "%roll", "" + rollingRadius);
        log(line);
    }
    ...
    // 補足: targetParams は [unitArea, allowClumps, usePixelCount] を想定する。
        ...
        if (useLabelMask && !useExclOn) {
            ...
            while (i + 2 < flatLen) {
                ...
                if (xi >= 0 && yi >= 0 && xi < w && yi < h) {
                    ...
                    // 団塊を代表面積から分割推定する（許可時かつピクセル計数でない場合のみ）。
                    est = 1;
                    if (usePixelCount == 1) {
                        est = a;
                    } else if (allowClumps) {
    ...
    function appendTuningText(line) {
        ...
        win = call("ij.WindowManager.getWindow", T_tune_text_title);
        if (win == 0 || ("" + win) == "null") {
            selectWindow(T_tune_text_title);
            print(line);
            return;
        }
        call("ij.text.TextWindow.append", win, line);
    }
    ...
    function runBatchAnalysis(analysisIdxList, forcePerCellStats, skipMissingRoiPrompt) {
        ...
        if (DEBUG_MODE == 1) {
            stirRandomByTime(analysisCount);
    ...
    if (lang == "中文") {
        ...
        T_param_spec_label = "参数字符串（覆盖下面设置）";
        T_param_spec_hint =
            "提示：此项非空时，将忽略下面所有参数，完全使用该字符串。\n" +
            "格式：key=value；用英文分号 “;” 分隔。";
        ...
        T_data_format_doc_cols =
            ...
            "  - 若列含 TPC/ETPC/TPCSEM，则按细胞展开；仅单细胞列随行变化。\n" +
        ...
        T_log_tune_setup = "  |- 调教数据：图像=%img 荧光=%fluo PN=%pn 时间点=%time 组=%group 有效组=%valid";
        T_log_tune_group_stats = "  |  - 组内样本：min=%min max=%max avg=%avg";
        T_log_tune_sample_mode = "  |- 调教抽样：%mode（组内最大样本=%max）";
        T_log_tune_sample_mode_single = "单次";
        T_log_tune_sample_mode_double = "双次";
        T_log_tune_sample = "  |  - 抽样集%set：count=%count";
        T_log_tune_score_detail = "  |  - 评分集%set：score=%score ratio=%ratio cv=%cv pn=%pn pairs=%pairs";
        T_log_tune_adjust = "  |  - 参数调整：minA=%min maxA=%max circ=%circ diff=%diff bg=%bg small=%small clump=%clump roll=%roll";
        ...
        T_param_spec_hint =
            ...
            "形式：key=value を英セミコロン “;” で区切ってください。";
        ...
        T_data_format_doc_cols =
            ...
            "  - TPC/ETPC/TPCSEM を含む場合は細胞ごとに 1 行（単細胞関連の列のみ変化）。\n" +
        ...
        T_log_tune_setup = "  |- チューニングデータ：画像=%img 蛍光=%fluo PN=%pn 時間点=%time グループ=%group 有効グループ=%valid";
        T_log_tune_group_stats = "  |  - グループ内サンプル：min=%min max=%max avg=%avg";
        T_log_tune_sample_mode = "  |- チューニング抽選：%mode（グループ最大=%max）";
        T_log_tune_sample_mode_single = "単回";
        T_log_tune_sample_mode_double = "2回";
        T_log_tune_sample = "  |  - サンプルセット%set：count=%count";
        T_log_tune_score_detail = "  |  - スコアセット%set：score=%score ratio=%ratio cv=%cv pn=%pn pairs=%pairs";
        T_log_tune_adjust = "  |  - パラメータ調整：minA=%min maxA=%max circ=%circ diff=%diff bg=%bg small=%small clump=%clump roll=%roll";
        ...
        T_debug_mode = "Debug mode (skip image analysis; generate random values)";
        ...
        T_data_format_doc_cols =
            ...
            "  - If TPC/ETPC/TPCSEM is included, rows expand per cell; only per-cell columns vary.\n" +
        ...
        T_log_tune_setup = "  |- Tuning data: images=%img fluo=%fluo pn=%pn time=%time groups=%group valid=%valid";
        T_log_tune_group_stats = "  |  - Group samples: min=%min max=%max avg=%avg";
        T_log_tune_sample_mode = "  |- Tuning sampling: %mode (group max=%max)";
        T_log_tune_sample_mode_single = "single";
        T_log_tune_sample_mode_double = "double";
        T_log_tune_sample = "  |  - Sample set %set: count=%count";
        T_log_tune_score_detail = "  |  - Score set %set: score=%score ratio=%ratio cv=%cv pn=%pn pairs=%pairs";
        T_log_tune_adjust = "  |  - Parameter adjust: minA=%min maxA=%max circ=%circ diff=%diff bg=%bg small=%small clump=%clump roll=%roll";
    ...
    dataFormatCols = "TB/BIC/CWB,name=\"Cell with Target Objects\"/TC/TPC/ETPC/TPCSEM";
    if (nTotalImgs > 1) {
        dataFormatCols = "T/" + dataFormatCols;
    }
    ...
    while (rerunFlag == 1) {
            ...
            if (dataFormatEnable == 1) {
                ...
                if (errMsg != "") {
                    ...
                    if (errTmp2 == "NaN" ||
                        ...
                        dataFormatCols = "TB/BIC/CWB,name=\"Cell with Target Objects\"/TC/TPC/ETPC/TPCSEM";
                        if (nTotalImgs > 1) dataFormatCols = "T/" + dataFormatCols;
                        errMsg = validateDataFormatCols(dataFormatCols);
                        errMsg = "" + errMsg;
                        if (errMsg == "0") errMsg = "";
                    }
        ...
        if (HAS_FLUO == 1 && tuneEnable == 1) {
            ...
            if (tuneOk == 1) {
                ...
                tuneGroupMax = 0;
                b = 0;
                while (b < tuneGroupLen.length) {
                    gLen = tuneGroupLen[b];
                    if (gLen > 0) {
                        tuneGroupTotal = tuneGroupTotal + gLen;
                        tuneGroupValid = tuneGroupValid + 1;
                        if (tuneGroupMin == 0 || gLen < tuneGroupMin) tuneGroupMin = gLen;
                        if (gLen > tuneGroupMax) tuneGroupMax = gLen;
                    }
                    b = b + 1;
                }
                tuneGroupAvg = 0;
                if (tuneGroupValid > 0) tuneGroupAvg = tuneGroupTotal / tuneGroupValid;

                line = T_log_tune_setup;
                line = replaceSafe(line, "%img", "" + nTotalImgs);
                line = replaceSafe(line, "%fluo", "" + tuneGroupTotal);
                line = replaceSafe(line, "%pn", "" + tunePnLen);
                line = replaceSafe(line, "%time", "" + nT);
                line = replaceSafe(line, "%group", "" + (tunePnLen * nT));
                line = replaceSafe(line, "%valid", "" + tuneGroupValid);
                log(line);

                line = T_log_tune_group_stats;
                line = replaceSafe(line, "%min", "" + tuneGroupMin);
                line = replaceSafe(line, "%max", "" + tuneGroupMax);
                line = replaceSafe(line, "%avg", "" + tuneGroupAvg);
                log(line);

                tuneNeedTwoSample = 1;
                modeLabel = T_log_tune_sample_mode_double;
                if (tuneGroupMax < 5) {
                    tuneNeedTwoSample = 0;
                    modeLabel = T_log_tune_sample_mode_single;
                }
                line = T_log_tune_sample_mode;
                line = replaceSafe(line, "%mode", modeLabel);
                line = replaceSafe(line, "%max", "" + tuneGroupMax);
                log(line);
                ...
                while (tuneLoop == 1) {
                    ...
                    while (iter < tuneRepeat) {
                        ...
                        line = T_log_tune_sample;
                        line = replaceSafe(line, "%set", "1");
                        line = replaceSafe(line, "%count", "" + idxList1.length);
                        log(line);
                        ...
                        line = T_log_tune_score_detail;
                        line = replaceSafe(line, "%set", "1");
                        line = replaceSafe(line, "%score", "" + stats1[1]);
                        line = replaceSafe(line, "%ratio", "" + stats1[2]);
                        line = replaceSafe(line, "%cv", "" + stats1[3]);
                        line = replaceSafe(line, "%pn", "" + stats1[4]);
                        line = replaceSafe(line, "%pairs", "" + stats1[5]);
                        log(line);

                        score = stats1[1];
                        ratioMean = stats1[2];
                        ratioCv = stats1[3];
                        if (tuneNeedTwoSample == 1) {
                            ...
                            line = T_log_tune_sample;
                            line = replaceSafe(line, "%set", "2");
                            line = replaceSafe(line, "%count", "" + idxList2.length);
                            log(line);
                            ...
                            line = T_log_tune_score_detail;
                            line = replaceSafe(line, "%set", "2");
                            line = replaceSafe(line, "%score", "" + stats2[1]);
                            line = replaceSafe(line, "%ratio", "" + stats2[2]);
                            line = replaceSafe(line, "%cv", "" + stats2[3]);
                            line = replaceSafe(line, "%pn", "" + stats2[4]);
                            line = replaceSafe(line, "%pairs", "" + stats2[5]);
                            log(line);
        ...
        if (roiPaths.length != nTotalImgs) roiPaths = newArray(nTotalImgs);
