macro "巨噬細胞画像 四要素解析 / Macrophage Four-Factor Analysis / マクロファージ四要素解析" {
    ...
    function isParamSpecKey(key) {
        ...
        while (i < keyOrder.length) {
            if (toLowerCase(keyOrder[i]) == key) return 1;
            i = i + 1;
        }
        return 0;
    }
    ...
    function getParamSpecOutputValue(keyLower) {
        ...
        if (keyLower == "strict") return strictKeyFromChoice(strictChoice);
    ...
    function applyParamSpec(spec, stage) {
        ...
        if (startsWith(sLower, "param_spec=")) {
    ...
    if (lang == "中文") {
        ...
        T_param_spec_hint =
            ...
            "导出的字符串固定包含全部参数键。留空表示跳过该键；0 会按有效值读取。";
        ...
        T_param_spec_hint =
            ...
            "出力文字列は常に全キーを含みます。空値は読み飛ばし、0 は有効値として読み取ります。";
        ...
        T_param_spec_hint =
            ...
            "The emitted string always contains all keys. Empty values are skipped; 0 is treated as a valid value.";
