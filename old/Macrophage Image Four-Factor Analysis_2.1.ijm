macro "巨噬細胞画像 四要素解�?/ Macrophage Four-Factor Analysis / マクロファージ四要素解析" {
    ...
    if (lang == "中文") {
        ...
        T_data_format_doc =
            "【数据格式化 - 规则说明】\n\n" +
            "A. 文件名识别规则（pn/f）：\n" +
            "  A.1 目的：从文件名中提取 项目名（pn�?�?编号（f）。\n" +
            "  A.2 语法：pn/f �?f/pn（必须包含且仅包含一个�?”）。\n" +
            "  A.3 代号说明：\n" +
            "      - pn：项目名（非数字部分）。\n" +
            "      - f ：编号为�?,2,3...”格式（不带前导 0）。\n" +
            "      - f1：编号为�?1,02,03...”或�?01,002,003...”等带前�?0 的格式。\n" +
            "  A.4 说明：f �?f1 仅用于识别编号格式，排序始终按数字大小。\n\n" +
            "B. 表格列格式：\n" +
            "  B.1 语法：列1/�?/�?（从左到右）。\n" +
            "  B.2 内置列代号（默认列名 / 含义）：\n" +
            "      - PN：PN / 项目名。\n" +
            ...
            "      - BiC：Beads in Cells / 细胞�?beads 数。\n" +
            "      - CwB：Cells with Beads / �?beads 的细胞数。\n" +
            "      - CwBa：Cells with Beads (Adj) / �?beads 的细胞数（微量校正）。\n" +
            "      - TC：Total Cells / 细胞总数。\n" +
            "      - IBR：IBR / BiC / TB。\n" +
            "      - PCR：PCR / CwB / TC。\n" +
            "      - eIBR：eIBR / 项目�?IBR 平均。\n" +
        ...
        T_err_df_rule_empty = "文件名识别规则为空。示例：pn/f �?f/pn";
        T_err_df_rule_slash = "文件名识别规则必须包含且仅包含一个�?”。示例：pn/f �?f/pn";
        T_err_df_rule_parts = "文件名识别规则的两部分都必须填写�?;
        T_err_df_rule_tokens = "文件名识别规则只允许“pn”和“f/f1(或以f开�?”两类标记�?;
        T_err_df_rule_need_both = "文件名识别规则必须同时包�?pn �?f�?;
        T_err_df_rule_order = "文件名识别规则顺序只允许 pn/f �?f/pn�?;
        ...
        T_data_format_doc =
            ...
            "  A.2 書式：pn/f また�?f/pn（�?”は1つだけ）。\n" +
            "  A.3 記号：\n" +
            "      - pn：プロジェクト名（数字以外の部分）。\n" +
            "      - f ：番号が�?,2,3...”の形式（先�?0 なし）。\n" +
            "      - f1：番号が�?1,02,03...”や�?01,002,003...”の形式（先�?0 あり）。\n" +
            "  A.4 補足：f �?f1 は番号の書式判定のみ。並び順は数値で判定します。\n\n" +
            "B. 表の列フォーマット：\n" +
            "  B.1 書式：列1/�?/�?（左から右）。\n" +
            "  B.2 内蔵列コード（既定の列名 / 意味）：\n" +
            "      - PN：PN / プロジェクト名。\n" +
            ...
            "      - BiC：Beads in Cells / 細胞�?beads 数。\n" +
            "      - CwB：Cells with Beads / beads を含む細胞数。\n" +
            "      - CwBa：Cells with Beads (Adj) / beads を含む細胞数（微量補正）。\n" +
            "      - TC：Total Cells / 細胞総数。\n" +
            "      - IBR：IBR / BiC / TB。\n" +
            "      - PCR：PCR / CwB / TC。\n" +
            "      - eIBR：eIBR / プロジェクト�?IBR 平均。\n" +
            "      - ePCR：ePCR / プロジェクト�?PCR 平均。\n" +
            "  B.3 カスタム列：任意のコード（内蔵列と重複不可）。\n" +
            "  B.4 パラメータ：カンマ区切り�?name=\"...\" また�?value=\"...\"。\n" +
            "      - name：列名（空白可）。\n" +
            "      - value：全行に固定値。\n" +
            "  B.5 �?”接頭：カスタム列のみ。列�?回だけ表示（PN 拡張しない）。\n\n" +
            "C. 記号注意：\n" +
        ...
        T_err_df_rule_empty = "ファイル名ルールが空です。例：pn/f また�?f/pn";
        T_err_df_rule_slash = "ファイル名ルールは�?”を1つだけ含めてください。例：pn/f また�?f/pn";
        T_err_df_rule_parts = "ファイル名ルール�?つの要素をどちらも入力してください�?;
        T_err_df_rule_tokens = "ファイル名ルールは“pn”と“f/f1(またはfで始まる)”のみ使用できます�?;
        T_err_df_rule_need_both = "ファイル名ルールには pn �?f の両方が必要です�?;
        T_err_df_rule_order = "ファイル名ルールの順序は pn/f また�?f/pn のみです�?;
        ...
        T_data_format_doc =
            ...
            "      - f : index in �?,2,3...�?format (no leading zeros).\n" +
            "      - f1: index in �?1,02,03...�?or �?01,002,003...�?format (leading zeros).\n" +
