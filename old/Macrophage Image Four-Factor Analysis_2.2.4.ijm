macro "巨噬細胞画像 四要素解�?/ Macrophage Four-Factor Analysis / マクロファージ四要素解析" {
    // =============================================================================
    // 概要: 巨噬細胞画像�?要素解析を行うImageJマクロ（Fiji専用�?    // 目的: ROI標注、対象物検出、統計集計、結果出力を一連の対話フローで実行する
    // 想定: Fiji上での実行とユーザー操作を含む（ImageJ単体では動作しない）
    // 署名: 西方研究室（nishikata lab�?/ wangsychn@outlook.com
    // 版数: 2.2.4 / ライセン�? CC0 1.0（本スクリプト）
    // 注意: 同梱のFiji/フォント等は各ライセンスに従う（THIRD_PARTY_NOTICES.md参照）�?    // =============================================================================

    // AI编辑提示：修改前请先阅读本仓库的AGENTS.md�?    // AIによる編集前に、このリポジトリのAGENTS.mdを必ず確認すること�?    // Note for AI contributors: Read AGENTS.md in this repository before editing.
    ...
    // -----------------------------------------------------------------------------
    // 関数: log
    // 概要: LOG_VERBOSEが有効なときのみログを出力する�?    // 引数: s (string) - 出力するメッセー�?    // 戻り�? なし
    // -----------------------------------------------------------------------------
    function log(s) {
    ...
    // 概要: substring の終端がinclusiveか判定する�?    // 引数: なし
    ...
    // -----------------------------------------------------------------------------
    // 関数: max2
    // 概要: 2値の最大値を返す�?    // 引数: a (number), b (number)
    // 戻り�? number
    // -----------------------------------------------------------------------------
    function max2(a, b) {
        if (a > b) return a;
    ...
    // -----------------------------------------------------------------------------
    // 関数: min2
    // 概要: 2値の最小値を返す�?    // 引数: a (number), b (number)
    // 戻り�? number
    // -----------------------------------------------------------------------------
    function min2(a, b) {
        if (a < b) return a;
    ...
    // -----------------------------------------------------------------------------
    // 関数: ceilInt
    // 概要: 切り上げ（負数対応）で整数化する�?    // 引数: x (number)
    // 戻り�? number
    // -----------------------------------------------------------------------------
    function ceilInt(x) {
        f = floor(x);
    ...
    // -----------------------------------------------------------------------------
    // 関数: clamp
    // 概要: [a, b] にクランプする�?    // 引数: x (number), a (number), b (number)
    // 戻り�? number
    // -----------------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------------
    // 関数: isImageFile
    // 概要: 画像拡張子（tif/tiff/png/jpg/jpeg）か判定する�?    // 引数: filename (string)
    // 戻り�? 1 = 画像, 0 = 非画�?    // -----------------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------------
    // 関数: getBaseName
    // 概要: 拡張子を除いたベース名を返す�?    // 引数: filename (string)
    // 戻り�? string
    // -----------------------------------------------------------------------------
    ...
    // 概要: パス末尾にスラッシュを付与する�?    // 引数: p (string)
    // 戻り�? string
    // -----------------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------------
    // 関数: splitByChar
    // 概要: 1文字区切りで文字列を分割する�?    // 引数: s (string), ch (string)
    // 戻り�? array
    // -----------------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------------
    // 関数: joinNumberList
    // 概要: 数値配列をカンマ区切りの文字列に変換する�?    // 引数: arr (array)
    // 戻り�? string
    // -----------------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------------
    // 関数: parseNumberList
    // 概要: カンマ区切りの数値文字列を配列に変換する�?    // 引数: s (string)
    // 戻り�? array
    // -----------------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------------
    // 関数: meanFromCsv
    // 概要: カンマ区切り数値の平均を返す（空は""）�?    // 引数: s (string)
    // 戻り�? number or ""
    // -----------------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------------
    // 関数: scaleCsv
    // 概要: カンマ区切り数値を係数でスケールする（四捨五入、負数は0）�?    // 引数: s (string), factor (number)
    // 戻り�? string
    // -----------------------------------------------------------------------------
    ...
    // 概要: CSV文字列配列の指定要素を係数でスケールして上書きする�?    // 引数: arr (array), idx (number), factor (number)
    // 戻り�? number (0)
    // -----------------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------------
    // 関数: buildCsvCache
    // 概要: CSV文字列配列をフラット配列に展開し開始/長さを記録する�?    // 引数: csvArr (array), startArr (array), lenArr (array)
    // 戻り�? array (flat)
    // -----------------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------------
    // 関数: meanFromCache
    // 概要: フラット配列キャッシュから平均値を計算する�?    // 引数: flat (array), startIdx (number), len (number)
    // 戻り�? number or ""
    // -----------------------------------------------------------------------------
    ...
    // 概要: フラット配列キャッシュの指定要素を係数でスケールする�?    // 引数: flat (array), startArr (array), lenArr (array), idx (number), factor (number)
    // 戻り�? number (0)
    // -----------------------------------------------------------------------------
    ...
    // 概要: フラット配列キャッシュから指定位置の値を取得する�?    // 引数: flat (array), startArr (array), lenArr (array), idx (number), cellIdx (number)
    // 戻り�? number or ""
    // -----------------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------------
    // 関数: buildZeroCsv
    // 概要: 指定数の0をカンマ区切りで生成する�?    // 引数: n (number)
    // 戻り�? string
    // -----------------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------------
    // 関数: getNumberAtCsv
    // 概要: カンマ区切り文字列の指定位置の数値を返す�?    // 引数: s (string), idx (number)
    // 戻り�? number or ""
    // -----------------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------------
    // 関数: splitCSV
    // 概要: クォート対応でカンマ区切りを分割する�?    // 引数: s (string)
    // 戻り�? array
    // -----------------------------------------------------------------------------
    function splitCSV(s) {
        arr = newArray();
    ...
    // -----------------------------------------------------------------------------
    // 関数: charAtCompat
    // 概要: substring仕様に合わせ�?文字を取得する�?    // 引数: s (string), idx (number)
    // 戻り�? string
    // -----------------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------------
    // 関数: isDigitAt
    // 概要: 文字列の指定位置が数字か判定する�?    // 引数: s (string), idx (number)
    // 戻り�? number (1/0)
    // -----------------------------------------------------------------------------
    ...
    // 概要: ルールトークンを正規化する�?    // 引数: part (string)
    // 戻り�? string ("p"/"f"/"")
    // -----------------------------------------------------------------------------
    ...
    // 概要: 文字列内の最初の連続数字を返す�?    // 引数: s (string)
    // 戻り�? string
    // -----------------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------------
    // 関数: parseRuleSpec
    // 概要: ルール指定文字列からパターンとF/T割当を抽出する�?    // 引数: spec (string), defaultTarget (string)
    ...
    // -----------------------------------------------------------------------------
    // 関数: parsePnF
    // 概要: ルールに従ってベース名からPN/Fを抽出し、F/T割当を返す�?    // 引数: base (string), ruleSpec (string), defaultTarget (string)
    ...
    // -----------------------------------------------------------------------------
    // 関数: isBuiltinToken
    // 概要: 組み込み列コードか判定する�?    // 引数: tokenKey (string, lower)
    // 戻り�? number (1/0)
    // -----------------------------------------------------------------------------
    ...
    // 概要: 列トークンを内部コードに変換する�?    // 引数: token (string)
    ...
    // 概要: ファイル名識別ルールの妥当性を検証する�?    // 引数: rule (string)
    ...
    // 概要: 表格列格式的妥当性を検証する�?    // 引数: cols (string)
    // 戻り�? string (空ならOK、それ以外はエラーメッセージ)
    // -----------------------------------------------------------------------------
    ...
    // 概要: 表格列配置が単細胞統計（BPC/EBPC/BPCSDP）を要求するか判定する�?    // 引数: cols (string)
    // 戻り�? number (1=必要, 0=不要)
    // -----------------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------------
    // 関数: findGroupIndex
    // 概要: (pn, keyNum) のグループインデックスを検索する�?    // 引数: pn (string), keyNum (number), groupPn (array), groupKey (array)
    ...
    // 概要: 数値配列と対応配列を昇�?降順でソートする�?    // 引数: nums (array), strs (array), desc (number)
    // 戻り�? なし（配列を直接並べ替える）
    // -----------------------------------------------------------------------------
    ...
    // 概要: 数値配列と対応2配列を昇�?降順でソートする�?    // 引数: nums (array), strs (array), idxs (array), desc (number)
    // 戻り�? なし（配列を直接並べ替える）
    // -----------------------------------------------------------------------------
    ...
    // 概要: 数値配列と対応3配列を昇�?降順でソートする�?    // 引数: nums (array), strs (array), idxs (array), ids2 (array), desc (number)
    // 戻り�? なし（配列を直接並べ替える）
    // -----------------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------------
    // 関数: calcRatio
    // 概要: 分子/分母を安全に計算する�?    // 引数: num (number), den (number)
    // 戻り�? number or ""
    // -----------------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------------
    // 関数: ensure2D
    // 概要: Zスタックの場合はスライス1に固定し2D化する�?    // 引数: なし
    // 戻り�? なし
    // -----------------------------------------------------------------------------
    function ensure2D() {
    ...
    // -----------------------------------------------------------------------------
    // 関数: safeClose
    // 概要: 指定ウィンドウが開いていれば閉じる�?    // 引数: title (string)
    // 戻り�? なし
    // -----------------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------------
    // 関数: escapeForReplace
    // 概要: replace() の置換文字列で問題になる "$" �?"\" をエスケープする�?    // 引数: s (string)
    // 戻り�? string
    // -----------------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------------
    // 関数: requireWindow
    // 概要: 指定ウィンドウが存在しない場合はエラー終了する�?    // 引数: title (string), stage (string), fileName (string)
    // 戻り�? なし
    // -----------------------------------------------------------------------------
    ...
    // 概要: データ形式の設定内容を詳細にログ出力する�?    // 引数: rule, cols, itemSpecs, itemTokens, itemNames, itemValues, itemSingles, sortDesc, sortKeyLabel
    // 戻り�? なし
    // -----------------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------------
    // 関数: maybePrintMotto
    // 概要: 言語設定とフラグに応じてランダムなモットーを表示する�?    // 引数: なし
    // 戻り�? なし
    // -----------------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------------
    // 関数: getPixelSafe
    // 概要: 座標を画像範囲にクランプしてピクセル値を取得する�?    // 引数: x (number), y (number), w (number), h (number)
    // 戻り�? number
    // -----------------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------------
    // 関数: localMean3x3
    // 概要: 3x3近傍の平均灰度を返す（境界は安全取得）�?    // 引数: x (number), y (number), w (number), h (number)
    // 戻り�? number
    // -----------------------------------------------------------------------------
    ...
    // 概要: 画像を開き、ROI Managerで細胞ROIを対話的に作�?編集してZIP保存する�?    // 引数: dir (string), imgName (string), roiSuffix (string), idx (number),
    //       total (number), skipFlag (number)
    // 戻り�? skipFlag (number) - 「以降をスキップ」状�?    // 副作�? 画像の表示、ROI Manager操作、ユーザー操作待ちが発生する�?    // -----------------------------------------------------------------------------
    ...
    // 概要: サンプル面積の分布から、外れ値に強い範囲と代表値を推定する�?    // 引数: sampleAreas (array), fallbackMin (number), fallbackMax (number)
    ...
    // 概要: 代表面積を直径に換算し、経験則でRolling Ball半径を推定する�?    // 引数: unitArea (number)
    // 戻り�? number
    // -----------------------------------------------------------------------------
    ...
    // 概要: 目標/排除サンプルの灰度分布から排除モードと閾値を推定する�?    // 引数: targetMeans (array), exclMeansAll (array)
    ...
    // 概要: 平均濃度サンプルから中央値を推定する�?    // 引数: meanArray (array)
    // 戻り�? number（不十分な場合は-1�?    // 補足: 飽和値を除外し、サンプル不足時�?1を返す�?    // -----------------------------------------------------------------------------
    ...
    // 概要: 絶対差分の分布から閾値を推定する�?    // 引数: diffArray (array), fallback (number), minThr (number),
    ...
    // 概要: 小さめ判定に使う面積比率を推定する�?    // 引数: areaArray (array), fallback (number)
    // 戻り�? number
    // -----------------------------------------------------------------------------
    ...
    // 概要: 団塊判定に使う面積比率を推定する�?    // 引数: areaArray (array), fallback (number)
    // 戻り�? number
    // -----------------------------------------------------------------------------
    ...
    // 概要: 塊サンプルの面積から団塊最小面積倍率を推定する�?    // 引数: clumpAreas (array), unitArea (number), fallback (number)
    // 戻り�? number
    // -----------------------------------------------------------------------------
    ...
    // 概要: ROIごとにラベル値を塗り分け�?6-bitマスクを生成する�?    // 引数: maskTitle (string), origID (number), w (number), h (number),
    ...
    // -----------------------------------------------------------------------------
    // 関数: sampleRingMean
    // 概要: 指定半径のリング上の平均灰度を返す�?    // 引数: cx (number), cy (number), r (number), w (number), h (number)
    // 戻り�? number
    // -----------------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------------
    // 関数: computeSpotStats
    // 概要: 円形候補の中�?外周/背景の濃度指標を計算する�?    // 引数: cx (number), cy (number), r (number), w (number), h (number)
    ...
    // 概要: 円形候補の特徴カテゴリを判定する�?    // 引数: centerDiff (number), bgDiff (number), area (number), unitArea (number),
    ...
    // 概要: 特徴選択の番号リストを作成する�?    // 引数: useF1 (number), useF2 (number), useF3 (number), useF4 (number),
    ...
    // 概要: 参照画像を開き、指定タイトルにリネームする�?    // 引数: url (string), refTitle (string)
    ...
    // 概要: 参照画像が開けない場合に代替案を表示する�?    // 引数: repoUrl (string)
    // 戻り�? なし
    // -----------------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------------
    // 関数: filterFlatByMask
    // 概要: マスク内にある候補を除外して返す�?    // 引数: flat (array), maskTitle (string), w (number), h (number), fileName (string)
    // 戻り�? array
    // -----------------------------------------------------------------------------
    ...
    // 概要: 濃暗な塊を抽出するマスクを作成する�?    // 引数: grayTitle (string), strictChoice (string), fileName (string)
    // 戻り�? string（マスク画像タイトル�?    // -----------------------------------------------------------------------------
    ...
    // 概要: 細胞内の高密度領域を抽出するマスクを作成する�?    // 引数: grayTitle (string), cellLabelTitle (string), HAS_LABEL_MASK (number),
    ...
    // 概要: マスク画像から塊候補を抽出する�?    // 引数: maskTitle (string), minArea (number), maxArea (number), fileName (string)
    // 戻り�? flat配列 [x1, y1, a1, ...]
    // -----------------------------------------------------------------------------
    ...
    // 概要: 2つの検出法（閾�?エッジ）で円形候補を抽出し、近接点を統合する�?    // 引数: grayTitle (string), strictChoice (string), targetParams (array),
    ...
    function detectBeadsFusion(
        ...
        // 検出ポリシーの決定（厳密度により統合条件を変える�?        policy = "UNION";
        ...
        // 手法A: 閾値ベースで円形候補を抽出す�?        safeClose("__bin_A");
        ...
        // 手法B: エッジ抽出ベースで候補を抽出す�?        safeClose("__bin_B");
        ...
        // 近接距離のしきい値を、代表面積から推定す�?        r = sqrt(beadUnitArea / PI);
        ...
        // 近傍にある候補�?点に統合し、優先的に面積の大きい点を残�?        j = 0;
        while (j < nB) {
            x = xB[j];
            y = yB[j];
            a = aB[j];
    ...
    // 概要: 特徴選択に基づき、円形候補と塊候補を統合して返す�?    // 引数: grayTitle (string), strictChoice (string), targetParams (array),
    ...
    // -----------------------------------------------------------------------------
    // 関数: countBeadsByFlat
    // 概要: 対象物検出結果を細胞ごとに集計し統計値を返す�?    // 引数: flat, cellLabelTitle, nCellsAll, imgParams, HAS_LABEL_MASK,
    ...
    function countBeadsByFlat(
        ...
        // ラベルマスクが無い場合はROI境界のキャッシュを作�?        if (!useLabelMask) {
        ...
        // 対象物候補を順に走査して除�?集計を行�?        if (useLabelMask && !useExclOn) {
            if (currWin != "label") {
            ...
            while (i + 2 < flatLen) {
                ...
                if (xi >= 0 && yi >= 0 && xi < w && yi < h) {
                    ...
                    // ラベルマスクがある場合はピクセル値で細胞IDを取得す�?                    if (useLabelMask) {
                        ...
                        // ラベルマスクが無い場合はROIに含まれるかを判定す�?                        if (currWin != "gray") {
                            selectWindow(grayTitle);
                            currWin = "gray";
                        }
        ...
        // 微量貪食のしきい値を推定し、調整後の細胞数を算出す�?        if (useMinPhago == 1) {
            nz = newArray();
            c = 0;
            while (c < nCells) {
    ...
    if (lang == "中文") {
        ...
        T_data_format_doc =
            "【数据格式化 - 代号速查】\n" +
            "A. 文件名规则（仅用于解析，不是列代号）：\n" +
            "  语法�?p>/<f> �?<f>/<p>（仅一个\"/\"）；子文件夹：folderRule//fileRule。\n" +
        ...
        T_err_df_rule_tokens = "[E104] 文件名识别规则只允许 <p> �?<f> 标记�?;
        ...
        T_err_df_fix_101 = "修正：填�?<p>/<f> �?<f>/<p>�?;
        T_err_df_fix_102 = "修正：只保留一个�?”�?;
        T_err_df_fix_103 = "修正：补齐�?”两侧内容�?;
        T_err_df_fix_104 = "修正：仅使用 <p> �?<f> 标记�?;
        ...
        T_data_format_doc =
            ...
            "  形式�?p>/<f> また�?<f>/<p>（\"/\"�?つ）。サブフォルダー：folderRule//fileRule。\n" +
        ...
        T_err_df_rule_parts = "[E103] ファイル名ルール�?つの要素をどちらも入力してください�?;
        T_err_df_rule_tokens = "[E104] ファイル名ルール�?<p> �?<f> のみ使用できます�?;
        ...
        T_err_df_fix_101 = "修正�?p>/<f> また�?<f>/<p> を入力してください�?;
        T_err_df_fix_102 = "修正：�?�?�?つのみ残してください�?;
        T_err_df_fix_103 = "修正：�?”の左右を入力してください�?;
        T_err_df_fix_104 = "修正�?p> �?<f> のみ使用してください�?;
    ...
    // 画像名とROIパスの対応表を作成す�?    roiPaths = newArray(nTotalImgs);
    k = 0;
    while (k < nTotalImgs) {
    ...
    while (s < nTotalImgs) {
        ...
        if (nR > 0) {
            // 8-bit画像でROIを計測して面積・平均灰度を収集す�?            safeClose("__tmp8_target");
            selectWindow(origTitle);
                ...
                if (nCellsSample > 0) {
                    ...
                    if (HAS_LABEL_MASK_SAMPLE == 1) {
                        ...
                        while (r < sampleCount) {
                            ...
                            decided = 0;
                            if (bw > 0 && bh > 0) {
                                // ROI内の粗いグリッドで細胞内比率を評価し�?0%閾値で早期判定する�?                                step = floor(min2(bw, bh) / 6);
                                if (step < 1) step = 1;
    ...
    if (HAS_MULTI_BEADS) {
        ...
        while (s < nTotalImgs) {
            ...
            if (nR > 0) {
                ...
                // 8-bit画像でROIを計測して灰度分布と面積の候補を収集す�?                safeClose("__tmp8_excl");
                selectWindow(origTitle);
    ...
    if (HAS_MULTI_BEADS) {
        ...
        // 排除対象の灰度分布から閾値と方向を推定す�?        exInfo = estimateExclusionSafe(targetMeans, exclMeansAll);
        exclMode = exInfo[1];
        exclThr = exInfo[2];
    ...
    rerunFlag = 1;
    // パラメータ確認完了後に「重新分析」選択で再表示するため、ループで制御する�?    while (rerunFlag == 1) {
        // -----------------------------------------------------------------------------
        ...
        // 厳密度に応じて実際の検出範囲を拡縮す�?        effMinArea = beadMinArea;
        ...
        while (k < nTotalImgs) {
            ...
            // 対象物検出用�?-bit画像を作成す�?            selectImage(origID);
            ...
            // 細胞ラベルマスクを生成す�?            cellLabelTitle = "__cellLabel";
            ...
            // 排除フィルタが有効な場合は画像ごとに閾値を微調整す�?            exclThrImg = exclThr;
        ...
        if (dataFormatEnable == 1) {
            ...
            if (hasTimeRule == 1) {
                nPn = pnLen;
                nT = timeNums.length;
                // Timeブロックごとの最大行数を算出し、PNごとの表を横並びにする�?                timeRowCount = newArray(nT);
                t = 0;
                while (t < nT) {
                    maxLen = 0;
                    p = 0;
