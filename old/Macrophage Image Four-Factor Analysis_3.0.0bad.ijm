macro "巨噬細胞画像 四要素解析 / Macrophage Four-Factor Analysis / マクロファージ四要素解析" {
    ...
    if (lang == "中文") {
        ...
        T_data_format_doc =
            ...
            "          CWBA=含目标细胞(校正) | TC=细胞总数\n" +
            "  比例类：IBR=BIC/TC | PCR=CWB/TC\n" +
            "  单细胞：BPC=每细胞目标数\n" +
            "  均值/标准差：EIBR/ISDP(IBR) | EPCR/PSDP(PCR) | EBPC/BPCSDP(BPC)\n\n" +
            "C. 自定义列：\n" +
            ...
            "  - 荧光模式下，TB/BIC/BPC/EBPC/BPCSDP 将自动追加前缀列（如 #BPC），无对应荧光图像则留空。\n" +
        ...
        T_log_fluo_sampling_start = "✓ 步骤：荧光颜色抽样";
        T_log_fluo_sampling_done = "✓ 完成：荧光颜色抽样";
        ...
        T_log_fluo_sampling_start = "✓ 手順：蛍光色サンプリング";
        T_log_fluo_sampling_done = "✓ 完了：蛍光色サンプリング";
        ...
        T_err_fluo_rgb_format =
            ...
            "Use the R/G/B format, e.g., 0/255/0.";
        T_err_fluo_rgb_range =
        ...
        T_log_fluo_sampling_start = "✓ Step: Fluorescence color sampling";
        T_log_fluo_sampling_done = "✓ Complete: Fluorescence color sampling";
        ...
        T_log_fluo_missing = "  │  ⚠ Missing fluorescence image: %f";
    ...
    if (imgRootFilesAll.length > 0 && subDirs.length > 0) {
    ...
    if (HAS_FLUO == 1) {
        ...
        while (fluoSamplingOk == 0) {
            ...
            fluoTargetRgbStr = rgbToString(fluoTargetR, fluoTargetG, fluoTargetB);
            fluoNearRgbStr = rgbToString(fluoNearR, fluoNearG, fluoNearB);
    ...
    while (rerunFlag == 1) {
        ...
        if (HAS_FLUO == 1) {
            ...
            rgb = parseRgbTriple(fluoTargetRgbStrUI, T_fluo_target_rgb, "param/step3");
            if (rgb[0] == 0) continue;
            fluoTargetR = rgb[1];
            fluoTargetG = rgb[2];
            fluoTargetB = rgb[3];

            rgb = parseRgbTriple(fluoNearRgbStrUI, T_fluo_near_rgb, "param/step3");
            if (rgb[0] == 0) continue;
            fluoNearR = rgb[1];
            fluoNearG = rgb[2];
            fluoNearB = rgb[3];
